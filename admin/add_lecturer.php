    ...
    if (!empty($_FILES['lecturers']['name']) && in_array($_FILES['lecturers']['type'], $fileMimes)) {
        ...
        while (($lecturerData = fgetcsv($LecturerCsv, 10000, ",")) !== FALSE) {
            ...
            if (mysqli_num_rows($check) > 0) {
                $sql = "UPDATE lecturers SET lecturer_code = '$lecturer_code', lec_status = '$status' WHERE lecturer_name = '$lecturer_name'";
                if (mysqli_query($conn, $sql)) {
                    $file_success = "Lecturers successfully updated";
                }
            } else {
                $sql = "INSERT INTO lecturers(lecturer_name, lecturer_code, lec_status)VALUES('$lecturer_name', '$lecturer_code', '$status')";
                if (mysqli_query($conn, $sql)) {
